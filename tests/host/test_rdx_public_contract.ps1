...
$contracts = @(
    ...
    @{ Name = 'file transfer active query remains available'; Text = $fileTransferServiceHeader; Pattern = 'rdx_err_t\s+rdx_file_transfer_get_active\s*\(\s*int\s*\*\s*out\s*\)\s*;' },
    @{ Name = 'file sync busy query remains available'; Text = $fileTransferServiceHeader; Pattern = 'rdx_err_t\s+rdx_file_transfer_get_sync_busy\s*\(\s*int\s*\*\s*out\s*\)\s*;' },
    ...
    @{ Name = 'legacy file transfer active query signature remains available'; Text = $protocolHeader; Pattern = 'u8\s+rdx_is_file_transfer_active\s*\(\s*void\s*\)\s*;' },
    @{ Name = 'legacy file sync busy query signature remains available'; Text = $protocolHeader; Pattern = 'u8\s+rdx_is_file_sync_busy\s*\(\s*void\s*\)\s*;' },
