...
def check_final_config(audit):
    ...
    for enabled, comm, byte_test in [(0, None, 0), (1, None, 0), (1, None, 1),
        ...
        path = audit / f'config-{enabled}-{comm}-{byte_test}.c'
        ...
        result = subprocess.run(base + [f'-DTCFG_T2620_FACTORY_USB_CDC_ENABLE={enabled}',
                                ...
                                capture_output=True, text=True)
        (audit / f'config-{enabled}-{comm}-{byte_test}.log').write_text(result.stdout + result.stderr, encoding='utf-8')
    ...
    print('PASS: actual SDK preprocessing: CDC/test off/on, MSC retained, endpoint layout, invalid test/tool combinations rejected')
