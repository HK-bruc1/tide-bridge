...
$source = Join-Path $PSScriptRoot 'test_rdx_p11_storage_service.c'
...
$output = Join-Path ([System.IO.Path]::GetTempPath()) (
    'rdx_p11_storage_service_' + [guid]::NewGuid().ToString('N'))
...
try {
    & $Compiler -std=c11 -Wall -Wextra -Werror `
        "-I$serviceStubs" "-I$commonStubs" $source -o $output
    if ($LASTEXITCODE -ne 0) {
        throw "P11 APP storage format Host compile failed: $LASTEXITCODE"
    }
    & $output
    if ($LASTEXITCODE -ne 0) {
        throw "P11 APP storage format Host test failed: $LASTEXITCODE"
    }
} finally {
