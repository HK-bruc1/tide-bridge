...
> 历史设计参考（2026-09-21 标注）：本文保留早期方案与阶段记录，不作为当前连接／按键行为的对接依据。当前已取消 CONFIG/HOGP 广播模式切换和 KEY5 三击切换；RDX 与 HID 独立就绪，可同链或分链共存。KEY1～KEY5 均支持 HID 按下／释放，KEY5 同时保留原录音手势；只有无 BLE 连接时才允许原本地业务，不能仅按 HID 是否断开判断离线。当前按键规则见 [2-6](2-6.HOGP长按持续输入实施方案.md)，APP 配键见 [A2](2-5.HOGP_APP端配键协议对齐文档A2.md)。下文旧连接规则、切换步骤和阶段结论仅代表当时方案。

> BLE 架构状态（2026-07-17）：本文早期章节中的 HOGP/Config 分时广播、配置模式切换、connection owner 和双 BLE Central 热配置设想已废止，不再代表当前固件。正式 BLE 架构以 [`2.BLE单连接多服务共存与统一广播方案_RDX私有服务与HOGP.md`](./2.BLE单连接多服务共存与统一广播方案_RDX私有服务与HOGP.md) 为准：RDX 与 HOGP 共用一个统一广播入口、一个 GATT Server 和最多一条 BLE 连接；任意当前中心均可访问 RDX，物理按键仅在 `CCC enabled && encrypted && !suspended` 时进入在线 HOGP 功能。本文其余内容仅作为产品技术栈的历史讨论保留。
