...
> 对齐版本：v1 Draft
> 固定入口：`*APP#custom#...#`
> 详细固件设计：[配键方案 A1](2-4.HOGP_APP按键键值下发方案A1.md)
...
1. 在就绪的 RDX owner 连接下查询设备能力和当前五键配置。
...
APP 通过当前就绪的 RDX owner 下发配键命令。RDX 与 HID 为独立能力，可位于同一连接或不同连接；不存在 CONFIG/HOGP 广播模式切换。
...
4. 仅具备 HID 能力的连接不能配键；同时具备 RDX 与 HID 能力的连接可通过 RDX 通道配键。
5. 配键成功后不得根据断连推测成功，必须以成功响应或 GET 回读结果为准。
6. 配键成功后无需切换模式；HID 满足通知订阅、加密、未挂起及业务接纳条件后，按保存的配置上报。
...
当前产品出厂默认映射为 KEY1～KEY5 对应 F13～F17（Keyboard Usage Page `0x07`，Usage `0x68`～`0x6C`，modifiers 为 `00`）。无有效持久化配置时使用此表，RESET_KEYMAP 也恢复此表；已有有效配置（包括全零禁用配置）仍优先，不因固件升级被覆盖。
...
| `0x09` | `NOT_AUTHORIZED` | 停止重试，重新完成 RDX 连接/鉴权 |
...
- 就绪的 RDX owner 可配键，仅 HID 或未授权连接被拒绝；
- KEY1～KEY5 按下上报、松开释放；释放失败后恢复发送的行为通过验证；
- KEY5 HID 与在线录音并行、短按不启动长按录音、全零映射仍保留录音手势；
- 同链和分链下的 HID／录音并行仍需实机验证。

### 15.3 当前待双方确认

1. T2620 默认映射 KEY1～KEY5 → F13～F17 的实机验证；测试向量不等于产品默认值。
