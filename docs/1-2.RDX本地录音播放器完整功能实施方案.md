...
> 文档状态：Phase 5 功能实现已完成；4 MiB 新主控下完整播放依赖、双备份打包及真机回归均已通过
>
...
这里的“已打通”同时包含功能、板载回归和固件打包结论。新主控片内 Flash 为 32 Mbit（4 MiB）；双备份模式下当前 `CODE0=0x102000`，单 Bank 边界为 `0x1FF000`，剩余 `0xFD000`（1012 KiB），完整底层 Opus 解码库可以进入可刷镜像。

本方案将它收敛为一个完整、可预测、可维护的本地录音播放器。针对当前产品，用户侧只需要支持：
...
本产品的 SN 和实际录音数量上限都只有数百，直接通过 UXFILE/DAT 按 SN 探测即可。播放器不再复制 SN 集合、建立动态链表或维护其他第二份播放列表。
...
本地播放不是由单个宏或单个插件独立完成。完整链路如下：

```text
raw 文件（80 B/20 ms）
    -> Source_Dev0: AUDIO_CODING_STENC_OPUS
    -> opus_stenc_dec_plug
    -> get_ogg_opus_ops()
    -> lib_ogg_opus_dec.a
    -> 48 kHz PCM -> DAC
...
| 配置 | 职责 | 播放要求 |
|---|---|---|
| `TCFG_RDX_LOCAL_PLAYBACK_ENABLE` | 播放器业务逻辑总开关 | 总开关 |
| `TCFG_DEC_STENC_OPUS_ENABLE` | 注册 `AUDIO_CODING_STENC_OPUS` 对应的 JLStream 解码入口/适配层 | 必须开启 |
| `TCFG_DEC_OGG_OPUS_ENABLE` | 引入实际底层 Opus 解码库及 raw/固定 CBR 输入能力 | 必须开启，且是主要代码体积来源 |
| `CONFIG_OGG_OPUS_DEC_SET_CBR_PACKET_LEN=80` | 告诉底层库按固定 80 字节包长取帧 | 必须保持 |

虽然 `TCFG_DEC_OGG_OPUS_ENABLE` 的名字包含 `OGG`，它在本 SDK 中也控制当前 headerless raw/CBR Opus 所使用的底层解码实现。`TCFG_DEC_STENC_OPUS_ENABLE` 不能替代它，反过来也不能省略 Stereo 入口插件。

项目覆盖文件应让两个解码宏跟随播放总开关，并在总开关开启但依赖不完整时通过 `#error` 阻止构建。RDX 录音在 `effect_dev2` 中直接调用 `get_opus_stenc_ops()`；2026-07-28 新主控真机 A/B 已确认录音和本地回听均不依赖 `opus_stenc_plug`，因此历史 `TCFG_STENC_OPUS_ENABLE` 宏及其强制保留分支已经删除。
...
1. Phase 1 已完成上下曲、默认最新曲目、EOF 停止和 key/app 门控收敛。
...
播放器只缓存 `count/max_sn` 和当前游标，不拥有文件元数据，也不维护独立播放列表。
...
- 文件本身已有稳定的单调 SN 排序键；
- 上一曲、下一曲只需要沿 SN 单向探测，不需要任意节点插入操作；
- DAT 更新后仍然必须重新校验链表。

本项目只保留一种表示方式：UXFILE/DAT 管理文件集合，播放器保存 `selected_sn/current_sn`，导航时沿 SN 增减查找下一个有效 DAT 项。
...
候选失败时继续寻找下一个候选。列表就绪后 `selected_sn` 可以表示尚未打开的默认曲目；只有文件和播放流都启动成功后，候选才能成为新的 `selected_sn/current_sn`。
...
开机后不应立即打开文件，也不应自动出声。DAT 列表就绪或首次命令完成懒刷新后，曲目游标默认选中最大有效 SN，播放位置仍为文件头：

```text
selected_sn       = max_valid_sn
current_sn        = 0
state             = STOPPED
position          = 0
```

没有录音时仍保持 `selected_sn = 0` 和 `UNREADY`。例如 `[123,456,789]` 就绪后默认指向 `789`：`play` 从 `789` 的文件头播放，`next` 以 `789` 为锚点切到 `456`，`prev` 以 `789` 为锚点回绕到 `123`。

`play` 只播放当前选择，`prev/next` 只从当前选择切换曲目，不再承担“尚未选曲时决定首次入口”的额外语义。

### 4.3 自然结束与手动回绕分离

手动上一曲、下一曲始终允许首尾回绕。自然播放结束不自动切换下一条，必须停止、保留当前 `selected_sn` 并将播放位置归零；下次 `play` 从该录音头部重播，`prev/next` 仍以该 SN 为锚点环形切换。

## 5. 文件导航

### 5.1 唯一方案：直接按 SN 探测

本产品 SN 最多只有数百，导航以 `selected_sn` 为锚点，通过 `rdx_uxfile_get_file_data_by_sn()` 沿目标方向逐个探测，遇到 SN 空洞或无效 DAT 项直接跳过。一条命令最多扫描 `1..max_sn` 一圈，不分配动态内存，也没有第二套导航或播放路径。播放器不长期保存 UXFILE 返回的内部指针，真正播放每个候选前都重新查询 DAT 项。

### 5.2 列表变化策略

以下事件必须使播放器缓存的 `count/max_sn` 失效：
...
默认最新选择在列表初次就绪、当前选择已被清除，以及本地新录音完成时建立。录音开始前已停止本地回听，因此录音完成后将 `selected_sn` 更新为新的最大有效 SN，下一次 `play` 可直接回听刚录好的文件。
...
|---|---|---|---|---|
| `prev/next` | 尝试刷新列表，就绪后先默认选中最大 SN，再据此环形切换 | 以 `selected_sn` 为锚点切换 | 切换并继续播放 | 等待排空或先 `stop` |
...
3. 从 `selected_sn` 的相邻节点开始寻找候选；列表初次就绪时 `selected_sn` 已默认为最大有效 SN
4. 尝试打开候选文件
5. 关闭旧播放流和旧文件
6. 为候选创建播放流并预填数据
7. 成功后提交 selected_sn/current_sn，进入 PLAYING
8. 失败则继续沿同方向寻找，最多一整圈
...
当前依靠单次导航的一圈上限防止重复访问。由于产品录音数量只有数百，不再增加坏 SN 索引。
...
`ff/fr` 不是另一套播放逻辑。它们只在当前 `selected_sn/current_sn` 对应的同一个 `pb_file` 内调用统一 Seek 事务，复用同一个 Source_Dev0、JLStream、数据泵和状态机。它们不进入环形导航、不切换 `selected_sn`、不跨文件，统一使用绝对目标执行 Seek：
...
当前硬件键位中 `NUM0 CLICK/LONG` 为上一曲/快退，`NUM1 CLICK/LONG` 为下一曲/快进，`NUM2 CLICK` 为音量加，`NUM3 CLICK` 为音量减，`NUM4 CLICK` 为播放/暂停切换，`NUM4 LONG/UP` 保留录音开关语义。Phase 3 第一版不启用音量长按重复，避免和系统按键重复速率、快进快退长按语义混在一起；如后续确需长按连续调音量，应继续复用 `APP_MSG_VOL_UP/DOWN` 重复事件，不在 RDX 播放器内累计音量。
...
文件句柄、pump timer 和输入缓冲是播放器模块唯一实例的运行资源，不构成另一份曲目列表。

约束：

- `selected_sn` 表示 DAT 中已选中的稳定逻辑曲目，初次就绪时可为尚未打开的最大有效 SN，停止时仍保留；
...
```text
[PB] list ready: count=12 max_sn=20 selected=20
...
- 提供公开状态结构体副本；
- 先初始化播放器，再初始化 UXFILE；随后调用 `rdx_playback_refresh_playlist()` 建立默认最新选择；
- 提供 `rdx_playback_on_record_created()`，在新录音完成后重新选择最大有效 SN；
- 保留 `TCFG_RDX_LOCAL_PLAYBACK_ENABLE` 模块边界。

### 15.2 `rdx_uxfile.h` 及其实现

- 继续使用 `count/max_sn/get_file_data_by_sn` 作为唯一导航数据源；
- 在新增、删除、同步、恢复和格式化后显式使播放列表缓存失效；
- 文件删除前通知播放器处理当前文件。
...
目标：上下曲、默认最新曲目和 EOF 停止成为稳定、独立、可验证的播放器能力。
...
- [x] 修正单文件自循环；
- [x] 抽出统一切歌事务；
- [x] 明确开机默认曲目：列表就绪后指向最大有效 SN，不自动播放；
...
- [x] 完成板载 SD NAND 设备回归：上一曲、下一曲符合环形区域语义；
...
1. **格式化未抢占本地播放。**
   `PROTOCOL_EVENT_CMD_SD_FORMAT` 和 `rdx_app_format_handle()` 原先会直接进入 `rdx_uxfile_sd_format()`，没有先通知本地播放器释放正在播放的文件句柄。整改后，协议任务先将格式化请求投递到 `app_core`，由 `rdx_app_format_handle()` 调用 `rdx_playback_invalidate_playlist(PB_PLAYLIST_FORMATTING)`，立即停止播放、清空选中项并进入 `UNREADY`，再执行格式化。

2. **录音完成后播放列表未显式失效。**
   新录音停止并写入 DAT 后，播放器原先只能依靠 `count/max_sn` 懒检测文件集合变化，无法严谨覆盖同数量替换或不重启立即导航新文件的边界。整改后，`rdx_record_run_exit()` 在 `rdx_uxfile_dat_1_save_gen()` 之后、确认 `RECORD_STATE_STOP` 时通知列表内容变化。录音模块通过 `Q_CALLBACK` 投递到 `app_core`，由 `rdx_app_playback_content_changed()` 使 UXFILE 元数据和播放列表失效，再由 `rdx_playback_on_record_created()` 将游标重新定位到最大有效 SN。

这两个整改保持了既定边界：格式化、删除和录音完成对播放器的修改都在 `app_core` 串行执行；没有引入动态播放列表、文件数据库副本或独立音量状态。录音完成后不需重启，下一次播放从刚录好的最大 SN 头部开始。

## 17. 测试方案

### 17.1 Host 侧契约测试

日常 host contract 只保留稳定的架构和安全边界，不冻结客户可随时调整的按键功能和曲目顺序。本地播放具体行为以源码审查、固件构建和板载回归为准；host 侧若检查，只限于：
...
| `[20]` | 20 | next | 20 |
| `[20]` | 20 | prev | 20 |
| `[123,456,789]` | 上电默认 789 | play | 789 |
| `[123,456,789]` | 上电默认 789 | next | 456 |
| `[123,456,789]` | 上电默认 789 | prev | 123 |
| `[3,8,11,20]` | 3 | next | 20 |
| `[3,8,11,20]` | 20 | prev | 3 |
| `[3,8,11,20]` | 8 | next | 3 |
| `[3,8,11,20]` | 8 | prev | 11 |
...
- 开机不自动播放；列表就绪后 `selected_sn` 默认为最大有效 SN，`play/next/prev` 都以它为锚点；
- 切歌失败不会提交错误游标；
- 坏文件不会导致死循环；
- 自然播放结束后停止并等待用户操作；
...
## 20. 收尾增补：单键播放/暂停切换（Phase 5）

> 实施状态（2026-07-15）：文档、代码、host 静态契约、固件构建和板载 SD NAND 设备回归已完成；收尾回归发现的删除后重录与暂停恢复问题已修复并通过回归。

曲目选择和播放位置是两个独立概念：`selected_sn` 始终是环形列表的曲目游标，`resume_frame` 只是同一 SN 暂停会话的位置。为了让用户能够暂时中断并继续回听，播放器保留两个确定性的底层命令：
...
不再增加第三个 `resume()` 公共接口。`play()` 根据状态表达“开始或继续播放”，`pause()` 只表达“暂停当前播放”，避免三个公共命令表达重叠语义。按键入口使用单一的 `APP_MSG_REC_PLAY_TOGGLE`，由 `rdx_playback_toggle()` 在 `app_core` 串行地选择上述两个底层命令。

### 20.1 产品语义

1. 上电后不自动打开文件、不自动出声；DAT 列表就绪后，`selected_sn` 默认为最大有效 SN。
2. `play`、`next` 和 `prev` 都以同一个 `selected_sn` 为参考，不再存在“尚未选曲时由首次命令决定入口”的特殊分支。
3. `STOPPED` 中执行 `play` 时，从 `selected_sn` 的 0 位置开始播放。
4. `PLAYING` 中执行 `pause` 后，保留当前 SN 和暂停帧位置，进入 `PAUSED`。
5. `PAUSED` 中执行 `play` 时，只恢复同一 SN，不改变环形游标，不隐式切曲。
6. `PAUSED` 中执行 `prev/next` 时，仍以 `selected_sn` 为锚点切换录音；新曲始终从 0 位置开始并立即进入 `PLAYING`，不继承旧曲的暂停位置。
...
- 暂停时应释放 pump、JLStream 和文件句柄，只保留 SN、帧位置和必要的时长缓存；这样不会长时间占用文件，也不会阻塞删除、格式化或存储恢复；
...
3. clamp 到 [0, duration_frames]，保存 resume_sn、resume_frame 和时长缓存
...
| `STOPPED` | 重新校验 `selected_sn`，从 0 开始播放 |
...
- `APP_MSG_REC_PLAY_TOGGLE` 由 `rdx_app.c` 在 `app_core` 中调用 `rdx_playback_toggle()`；
- 离线模式（两个 BLE wrapper 均空闲）下，`KEY_IO_NUM4`/KEY5 的单击 `KEY_ACTION_CLICK` 发送该消息，是唯一的本地录音播放/暂停按键；
- `KEY_IO_NUM0` 和 `KEY_IO_NUM1` 的双击均不分配播放动作；
- `rdx_playback_toggle()` 在 `PLAYING/DRAINING` 时调用 `pause()`，在 `PAUSED/STOPPED/UNREADY` 时调用 `play()`，在 `STARTING/SWITCHING` 时返回 `BUSY`，不在状态转换期间反向操作；
- `rdx_key.c` 只做物理动作到单一消息的映射，不读取播放状态；
- 独立的暂停消息已删除；后续 app message 按当前枚举顺序自然递增；
- 不复用 `APP_MSG_REC_PREV/NEXT`，`KEY_IO_NUM0` 的 CLICK/LONG 仍为上一曲/快退，`KEY_IO_NUM1` 的 CLICK/LONG 仍为下一曲/快进；
...
- 协议文件删除会复制 SN/文件名和 RDX owner token 后投递到 `app_core`，先关闭命中的播放文件，再将删除交给 UXFILE worker；
- 普通协议格式化以及两种解绑格式化入口均投递到 `app_core`，二次校验录音、OTA、传输和索引状态后先停止播放再格式化；
- 格式化通过 `PB_PLAYLIST_FORMATTING` 清除选中项和暂停游标；正常关机在释放 UXFILE 任务和关闭存储电源前，通过 `PB_PLAYLIST_STORAGE_UNAVAILABLE` 关闭播放器资源；
...
Phase 5 实施时使用 `tests/host/test_rdx_playback_navigation.ps1` 冻结新接口、暂停游标、恢复回退与切曲后游标清理顺序；该脚本及其 30 项检查现作为历史阶段证据，不再属于日常 host 套件。当前软件验证统一通过 `tests/host/run_host_tests.ps1` 执行五组核心合同，本地播放导航行为继续由板载回归覆盖。
...
| `[123,456,789]` 上电列表就绪 | `play` | 默认选中的 `789` 从 0 开始播放 |
| `[123,456,789]` 上电列表就绪 | `next` | 以 `789` 为锚点切到 `456`，从 0 开始播放 |
| `[123,456,789]` 上电列表就绪 | `prev` | 以 `789` 为锚点回绕到 `123`，从 0 开始播放 |
...
| 本地新录音完成 | `play` | 游标已指向新的最大有效 SN，从文件头开始播放 |
| `PAUSED` | 文件同步/传输 busy | 拒绝新的播放命令，不自动恢复；库侧启动回调为设备回归待确认边界 |
| 离线 `KEY_IO_NUM4`/KEY5 单击 | `APP_MSG_REC_PLAY_TOGGLE` | 播放中暂停；暂停后恢复；停止时从已选曲目头部开始播放，不触发切曲 |

Phase 5 完成的验收底线：

- 暂停后不再继续明显出声，恢复时不播放暂停前的旧缓冲片段；
- `play/pause` 不改变上下曲方向、首尾回绕和当前 `selected_sn`；
...
1. APP 删除请求复制参数并投递到 `app_core`；UXFILE 接受删除后，调用 `rdx_playback_on_file_deleted(sn)` 停止当前播放、清除匹配的选中项/暂停游标并使播放列表失效。删除是异步 worker 任务，应由 UXFILE 拥有 DAT 缓存直到删除落盘，上层不在入队后立即释放该缓存。
2. 新录音停止并完成 DAT 保存后，`rdx_app_playback_content_changed()` 使 UXFILE DAT 缓存和播放列表失效，并重新选中最大有效 SN。
...
- 历史 `test_rdx_playback_navigation.ps1` 曾增加“恢复必须先 Seek 再开流”和“删除/重录必须使 UXFILE 元数据缓存失效”契约，阶段性播放导航 30 项检查全部通过；该脚本现已退出日常 host 套件；
- 当前五组 host 核心合同全部通过，固件构建通过，板载回归测试通过。

## 21. 2026-07-28 配置回归与芯片迁移约束

### 21.1 为什么恢复功能后出现杂音

提交 `918460ec7a1eafbe308d32b3fad1e5bdb7138421` 为释放代码空间，同时关闭了 `TCFG_RDX_LOCAL_PLAYBACK_ENABLE` 和 `TCFG_DEC_OGG_OPUS_ENABLE`。之后若只恢复本地播放总开关，`TCFG_DEC_STENC_OPUS_ENABLE` 虽会启用 Stereo 入口插件，但底层 `lib_ogg_opus_dec.a` 的 raw/CBR 解码能力仍未完整进入镜像，所以链路能够启动却输出杂音。

异常回归中，`301920` 字节文件理论时长为 `301920 / 4000 = 75.48 s`，实际约 `50.47 s` 结束；同时文件读取和写入字节数完整、无 SD 错误。该证据把问题限定在压缩帧解释/解码层，而不是播放器导航、数据泵或存储层。

### 21.2 容量迁移结论

以下数据是 16 Mbit（2 MiB）旧容量配置下的历史 A/B，用于说明完整解码依赖的空间增量：

| 构建组合 | 历史结果 |
|---|---|
| 播放关闭、底层解码库关闭 | `FLASH_BIN_SIZE=0xF5000`，剩余 `0xA000`（40 KiB） |
| 只恢复播放逻辑 | `FLASH_BIN_SIZE=0xF6000`，剩余 `0x9000`（36 KiB），但播放错误 |
| 完整恢复播放和解码依赖 | `CODE0=0x102000`，超过 `0xFF000` 边界 `0x3000`（12 KiB） |

完整解码配置相对“只有播放逻辑”的错误组合增长约 `0xC000`（48 KiB），主要来自 `TCFG_DEC_OGG_OPUS_ENABLE` 引入的底层 Opus 解码库。播放器逻辑本身约增加 `0x1000`（4 KiB）。

`0xFF000` 是旧 2 MiB 配置在 `TCFG_DUAL_BANK_ENABLE=1` 下的单 Bank OTA 代码边界，当时不能通过简单修改边界规避。更换 32 Mbit（4 MiB）新主控后，工程按真实容量设置 `CONFIG_FLASH_SIZE=0x400000`，并将尾部 VM 起始地址设置为 `0x3FC000`。

当前完整播放配置的结果为：`CODE0=0x102000`、`CODE_BOUNDARY_LINE=0x1FF000`、`LAVE_SIZE=0xFD000`。即单 Bank 上限 2044 KiB，已使用 1032 KiB，剩余 1012 KiB，使用率约 50.49%。固件已经成功打包，原 12 KiB 超界问题关闭。

### 21.3 迁移后验收结果

1. 新主控 Flash 容量、VM 和 BTIF 尾部分区已按 4 MiB 布局生成，双备份镜像打包通过。
2. 本地播放总开关、Stereo 解码入口和底层 OGG/raw Opus 解码库保持联动开启，原杂音问题不再复现。
3. 删除 `TCFG_STENC_OPUS_ENABLE` 后，RDX 新录音和耳机本地回听正常，确认编码链路不依赖 `opus_stenc_plug`。
4. `sdk_used_list.c` 已删除对应强制保留分支，host 契约固定 `effect_dev2 -> get_opus_stenc_ops()` 直接编码路径。
5. 当前仍需按产品发布流程持续回归上一曲、下一曲、Seek、暂停恢复、删除后重录及存储互斥场景；这些属于播放器可靠性回归，不再是芯片迁移或编码宏删除的阻塞项。
