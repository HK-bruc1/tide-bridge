...
- disconnect、HID ready->not ready、deinit、关机清理 active 并 best-effort release；
- release 失败记录错误并走 HOGP 既有 reset；
- KEY5 三击不再承担 BLE 模式切换，按当前离线 key table/产品按键策略处理；KEY5 长按/HOLD 的 PTT 只在 HID ready 时生效；
...
使用统一品牌前缀，并为 BLE 统一端点与 Classic HFP 端点使用不同后缀：

```text
BLE RDX + HOGP：VibeCoding Keyboard
Classic HFP：VibeCoding Mic
```

如果最终品牌为 Beanstalk，则两个端点统一替换前缀。禁止代码和文档混用 `Beanstalk`、`VibeKeyboard`、`VibeCoding`。
...
BLE 统一广播名与 GAP `0x2A00` 必须一致。PC Agent 不得仅通过名称关联 Keyboard 和 Mic，名称只用于用户识别。
...
4. KEY5 三击不再切换 BLE 模式；KEY5 长按/HOLD 仅在 HID ready 时用于 PTT，HID not ready 时继续走离线按键路径。
5. executor 合成持久 held report 与普通 click transient report，click timer 恢复 held。
6. disconnect/HID ready 丢失/deinit 清理 active。
...
- 覆盖 F24 held 期间 HOGP disconnect、CCC 关闭、suspend、加密失效、press 失败和重复 LONG/HOLD；
- 冻结“KEY5 三击不切换 BLE 模式”契约；HID not ready 时三击不得被 HOGP 路径吞掉；
...
KEY1～KEY4、KEY5 短按/三击、自定义 keymap、HID ready/not ready 切换、离线按键、HFP 音频。
...
- [ ] disconnect/HID ready 丢失/deinit 均清理 active。
- [ ] KEY5 三击不切换 BLE 模式；HID not ready 时仍走离线按键策略。
