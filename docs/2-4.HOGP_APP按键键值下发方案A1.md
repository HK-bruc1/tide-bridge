...
| PC / HOGP Host | 用户按键产生的标准 8-byte Keyboard Input Report | `rdx_hogp_input.c` 消费边沿后调用 `press/release` | HOGP Input Report characteristic ATT Notify |
...
物理按键向电脑发送键盘动作时不经过 RDX `custom`，由消抖扫描、私有 FIFO 和 app_core 复核后调用：

```c
rdx_hogp_key_action_press(key_id, input_epoch); /* key_id: 0..4 */
rdx_hogp_key_action_release(key_id);           /* 实际松开时 */
```

按下读取当前已生效 keymap 并发送 Report，持续按住期间保持状态，实际松开时发送全零释放 Report；不再定时自动释放。发送失败、断连、配键更新等按输入状态机处理，不能绕过 FIFO、输入代次和 HID token 校验直接调用。KEY5 的录音手势另走原业务路径，可以与 HID 并行。
...
Frame CRC32 是 v1 强制字段，用于发现 custom 内容损坏并作为请求指纹；即使 RDX 外层包也启用 CRC，该字段仍保留，使本协议不依赖预编译库是否向应用暴露外层校验结果。canonical keymap CRC 和 VM record CRC 采用同一算法，但覆盖范围不同。所有 CRC 都不代表身份认证，配置写入仍必须受 RDX owner 和产品鉴权状态约束。
...
当前使用统一广播，RDX 与 HID 独立就绪，无需切换广播身份或断开 BLE。推荐顺序：
...
以 ACK 或 GET 回读判断成功，不以断连推测成功。HID 按键和 RDX 配键可以在同一连接或分离连接上工作。
...
- 连接处于产品允许的绑定/鉴权状态；
- 当前不处于 OTA、关机或其他互斥事务；
- HID 属性不提供配键写入；同链 RDX_HID owner 可通过 RDX 通道配键，仅 HID 能力不能配键。

CRC 只能防止传输或存储损坏，不能代替鉴权。

当前以独立 RDX session/token 校验配置请求及提交阶段的归属，不再使用旧的 CONFIG/HOGP connection owner。HID owner 独立管理，不能仅因连接具备 HID 能力就拒绝其合法的 RDX 配键请求。鉴权与异常路径仍须按量产验收要求验证。
...
产品默认映射为 KEY1～KEY5 对应 F13～F17，使用 Keyboard Usage Page `0x07`，不带修饰键：

| 产品键 | 默认 modifiers | 默认 usages[6] | 状态 |
|---|---:|---|---|
| KEY1 | `00` | `68 00 00 00 00 00` | F13 |
| KEY2 | `00` | `69 00 00 00 00 00` | F14 |
| KEY3 | `00` | `6A 00 00 00 00 00` | F15 |
| KEY4 | `00` | `6B 00 00 00 00 00` | F16 |
| KEY5 | `00` | `6C 00 00 00 00 00` | F17 |

本文第 8 节的 Ctrl+C/Ctrl+V/Ctrl+X/Backspace/Enter 仅是协议测试向量，不是量产默认表。无有效持久化配置时使用上述默认表；RESET_KEYMAP 也恢复此表。已有有效配置（包括 APP 保存的全零禁用配置）优先，升级固件不会自动覆盖，需通过 APP 重新配键或 RESET_KEYMAP 切换。APP 监听这些标准键值并定义业务功能，固件不保存业务语义。
...
- 就绪的 RDX owner 可以 GET/SET；
- 仅具备 HID 能力的连接拒绝配置写入，同链 RDX_HID owner 可经 RDX 通道配键；
- 未绑定/未鉴权连接按产品策略拒绝写入；
- SET 成功 ACK 发出前不切换模式；
