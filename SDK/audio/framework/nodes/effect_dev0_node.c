 ...
 * 非0 : 按照帧长输入数据到算法处理接口
 */
#define EFFECT_DEV0_FRAME_POINTS  320 //(256)
 ...
 * */
static u32 audio_effect_dev0_run(struct effect_dev0_node_hdl *hdl, s16 *indata, s16 *outdata, u32 indata_len)
{
#if 0
    //test 2to4
...
static int effect_dev0_ioc_negotiate(struct stream_iport *iport)
{
    ...
    printf(" effecs_dev0 in_ch_num %d, out_ch_num %d\n", hdl->dev.in_ch_num, hdl->dev.out_ch_num);
...
static void effect_dev0_ioc_start(struct effect_dev0_node_hdl *hdl)
{
    ...
    hdl->dev.effect_run = (u32 (*)(void *, s16 *, s16 *, u32))audio_effect_dev0_run;
