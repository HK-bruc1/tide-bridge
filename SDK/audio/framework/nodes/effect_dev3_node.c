 ...
 * 非0 : 按照帧长输入数据到算法处理接口
 */
#define EFFECT_DEV3_FRAME_POINTS  (320) //(256)
 ...
 **/
static void audio_effect_dev3_init(struct effect_dev3_node_hdl *hdl)
{
    //do something
    ...
    temp = malloc(EFFECT_DEV3_FRAME_POINTS * 2 * 2);
    ASSERT(hdl->sw_src_buf);
    RS_PARA_STRUCT rs_para_obj;
    rs_para_obj.nch = hdl->dev.in_ch_num;
 ...
 * */
static u32 audio_effect_dev3_run(struct effect_dev3_node_hdl *hdl, s16 *indata, s16 *outdata, u32 indata_len)
{
#if 0
    //test 2to4
...
#endif
    //do something
    if(source_dev1_get_idx_enable(0) == 0){
        return indata_len;
    }
    u8 *data = (u8 *)indata;
    ...
    hdl->src_busy = 0;
    source_dev1_input_write(0, data, data_len);
    /* printf("effect dev3 do something here\n"); */
    return indata_len;
}
/* 自定义算法，关闭
 **/
static void audio_effect_dev3_exit(struct effect_dev3_node_hdl *hdl)
{
    //do something
    int wait = 0;
...
/* 自定义算法，更新参数
 **/
static void audio_effect_dev3_update(struct effect_dev3_node_hdl *hdl)
{
    //打印在线调音发送下来的参数
    printf("effect dev3 name : %s \n", hdl->name);
    for (int i = 0 ; i < 8; i++) {
...
static void effect_dev3_handle_frame(struct stream_iport *iport, struct stream_note *note)
{
    ...
    struct effect_dev3_node_hdl *hdl = (struct effect_dev3_node_hdl *)iport->node->private_data;
...
{
    struct effect_dev3_node_hdl *hdl = (struct effect_dev3_node_hdl *)node->private_data;
...
static int effect_dev3_ioc_negotiate(struct stream_iport *iport)
{
    ...
    printf(" effecs_dev3 in_ch_num %d, out_ch_num %d\n", hdl->dev.in_ch_num, hdl->dev.out_ch_num);
...
static void effect_dev3_ioc_start(struct effect_dev3_node_hdl *hdl)
{
    ...
    if (config_audio_cfg_online_enable) {
        if (jlstream_read_effects_online_param(hdl_node(hdl)->uuid, hdl->name, &hdl->cfg, sizeof(hdl->cfg))) {
            log_debug("get effect dev3 online param\n");
        }
    }
    printf("effect dev3 name : %s \n", hdl->name);
    for (int i = 0 ; i < 8; i++) {
    ...
    printf("effect_dev3_ioc_start, sr: %d, in_ch: %d, out_ch: %d, bitw: %d, %d", hdl->dev.sample_rate, hdl->dev.in_ch_num, hdl->dev.out_ch_num, hdl->dev.bit_width, hdl->dev.qval);


    hdl->dev.node_hdl = hdl;
    hdl->dev.effect_run = (u32 (*)(void *, s16 *, s16 *, u32))audio_effect_dev3_run;
    effect_dev_init(&hdl->dev, EFFECT_DEV3_FRAME_POINTS);

    audio_effect_dev3_init(hdl);
}
...
static int effect_ioc_update_parm(struct effect_dev3_node_hdl *hdl, int parm)
{
    int ret = false;
...
REGISTER_STREAM_NODE_ADAPTER(effect_dev3_node_adapter) = {
    ...
    .hdl_size   = sizeof(struct effect_dev3_node_hdl),
