    ...
    if (hdl->remain_buf) {
        ...
        if (pack_frame_num) {
            ...
            while (pack_frame_num--) {
                out_len += hdl->effect_run(hdl->node_hdl, (s16 *)((int)hdl->pack_frame + offset), (s16 *)((int)hdl->out_frame->data + out_offset), hdl->frame_len);
                offset += hdl->frame_len;
                out_offset = hdl->out_ch_num * offset / hdl->in_ch_num;
            }
            hdl->remain_len -= pack_frame_len;//剩余数据长度
            hdl->out_frame->len = out_len;//out_frame_len;
        ...
        if (hdl->out_ch_num != hdl->in_ch_num) {
            ...
            out_len = hdl->effect_run(hdl->node_hdl, (s16 *)frame->data, (s16 *)out_frame->data, frame->len);
            out_frame->len = out_len;
            jlstream_push_frame(node->oport, out_frame);	//将数据推到oport
            jlstream_free_frame(frame);	//释放iport资源
        } else {
            frame->len = hdl->effect_run(hdl->node_hdl, (s16 *)frame->data, (s16 *)frame->data, frame->len);
