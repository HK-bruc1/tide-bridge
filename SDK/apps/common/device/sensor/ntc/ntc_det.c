...
#define NTC_DET_PULLUP_TYPE     (0) //dons++
#endif
    ...
    if (ntc_det.cnt >= NTC_DET_CNT) {
        if (ntc_det.normal_cnt > NTC_DET_CNT / 2) {
            if (ntc_det.state == NTC_STATE_ABNORMAL) {
                ...
                // charge_start(); dons++
                extern void rdx_app_charge_start(void);
            ...
            if (ntc_det.state == NTC_STATE_NORMAL) {
                ...
                // charge_close(); dons++
                extern void rdx_app_charge_stop(void);
...
{
    r_printf("%s --> msg[0] = %d \r", __func__, msg[0]);
    switch (msg[0]) {
...
#define NTC_NORMAL_DETECT_TIMEOUT           (5*1000U)

#define NTC_NORMAL_DET_UPPER                940  //正常范围AD值上限，-20度时
#define NTC_NORMAL_DET_LOWER                186 //正常范围AD值下限，60度时
...
static NTC_Detect ntc_normal_detect;

static void ntc_normal_det_timer_deal(void *priv)
{
    u16 value[10];
    u32 avr_value = 0;
    for(int i=0;i<10;i++){
        value[i] = adc_get_value(NTC_DET_AD_CH);
        avr_value += value[i];
    }
    avr_value = avr_value/10;
    y_printf("=== %s --> ADC avr_value = %d \r", __func__, avr_value);

    if(avr_value < NTC_NORMAL_DET_LOWER || avr_value > NTC_NORMAL_DET_UPPER){
        if(ntc_normal_detect.cnt > NTC_DET_CNT){
            //alert, do system shutdown.
            ntc_normal_detect.cnt = 0;
            y_printf("=== %s --> ALERT, do system shudown! \r", __func__);
            sys_enter_soft_poweroff(POWEROFF_NORMAL);
        }else{
            ntc_normal_detect.cnt++;
        }
    }else{
        ntc_normal_detect.cnt = 0;
    }
...
void ntc_normal_det_stop(void)
{
    y_printf("%s", __func__);
    if(ntc_normal_detect.timer){
        sys_timer_del(ntc_normal_detect.timer);
    }
}

void ntc_normal_det_start(void)
{
    y_printf("%s", __func__);

    if(ntc_det_working()){
        y_printf("ntc detect working now, normal detect start fail. \r");
        return;
    }
    
    memset(&ntc_normal_detect, 0, sizeof(NTC_Detect));
    if (ntc_normal_detect.timer == 0) {
        printf("ntc normal detect start");
        gpio_set_mode(IO_PORT_SPILT(NTC_POWER_IO), PORT_OUTPUT_HIGH);
        gpio_set_mode(IO_PORT_SPILT(NTC_DETECT_IO), PORT_HIGHZ);
        adc_add_sample_ch(NTC_DET_AD_CH);
        ntc_normal_detect.timer = sys_timer_add(NULL, ntc_normal_det_timer_deal, NTC_NORMAL_DETECT_TIMEOUT);
    }
}
#endif
