...
#endif/* #if TCFG_APP_CAT1_EN && TCFG_CAT1_FUNC_NET_ENABLE */
...
extern const int ntp_get_time_init;
    ...
    if ((++(*p_to))*DHCP_TMR_INTERVAL >  dhcp_timeout_msec) {
        ...
        if (dhcp->state == DHCP_STATE_BOUND) {
            ...
            if (netif == &wireless_netif) {
                ...
                /* extern void websocket_client_thread_create(void); */
...
#ifdef HAVE_BT_NETIF
    if (netif == &bt_netif) {
        bt_lwip_event_cb(NULL, LWIP_BT_DHCP_BOUND_RELEASE);
    }
#endif
 ...
 * 有线网络事件处理
 */
static void wired_event_handler(struct device_event *event)
{
    switch (event->event) {
