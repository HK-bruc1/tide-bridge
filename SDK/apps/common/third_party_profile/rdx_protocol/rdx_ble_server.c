                                                  ...
                                                  uint16_t size)
{
    ...
    case ATT_EVENT_CAN_SEND_NOW:
        r_printf("[BLE_PHASE0A] can_send_now wrapper=%u cb_hdl=%p wrapper_con=0x%04x ignored\n",
                 rdx_ble_server_phase0a_wrapper_index(hdl), hdl,
                 hdl ? app_ble_get_hdl_con_handle(hdl) : 0);
        break;
...
{
#if TCFG_RDX_HOGP_DUAL_LINK_ENABLE
    r_printf("[BLE_PHASE0A] sm_event wrapper=%u cb_hdl=%p wrapper_con=0x%04x type=0x%02x channel=0x%04x size=%u ignored\n",
             rdx_ble_server_phase0a_wrapper_index(hdl), hdl,
             hdl ? app_ble_get_hdl_con_handle(hdl) : 0,
...
static uint16_t rdx_ble_server_att_read_callback(void *hdl, hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    if (!rdx_ble_session_is_current(connection_handle)) {
        y_printf("[BLE_SESSION] read ignored: stale hdl=0x%04x att=0x%04x\n",
                 connection_handle, handle);
        return 0;
    }
    if (rdx_ble_server_is_rdx_capability_handle(handle) &&
        !rdx_ble_server_rdx_attach(connection_handle)) {
        return 0;
    }
#endif
...
#if TCFG_RDX_HOGP_DUAL_LINK_ENABLE && TCFG_RDX_HOGP_ENABLE
...
static int rdx_ble_server_att_write_callback(void *hdl, hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    (void)buffer_size;
#endif
    r_printf("[BLE_PHASE0A] write rejected att=0x%04x; business runtime detached\n",
             handle);
    return RDX_BLE_PHASE0A_ATT_ERR_UNLIKELY_ERROR;
#else
    if (!rdx_ble_session_is_current(connection_handle)) {
        y_printf("[BLE_SESSION] write ignored: stale hdl=0x%04x att=0x%04x\n",
                 connection_handle, handle);
        return 0;
    }
    if (rdx_ble_server_is_rdx_capability_handle(handle) &&
...
#if TCFG_RDX_HOGP_ENABLE
    /* HID access attaches the HID capability to the current shared link. */
...
#else
    return rdx_ble_server_adv_enable_on_hdl(
        g_rdx_ble_server_info.rdx_ble_server_hdl, enable);
#endif
}
...
int rdx_ble_server_send(u8 *data, u32 len)
{
    ...
    if (!rdx_ble_session_is_rdx_active(
            g_rdx_ble_server_info.ble_con_handle)) {
        y_printf("[RDX_SESSION] send skipped: capability not active\n");
        /* A HID-only link is a valid routing state, not a transport failure. */
        return -1;
    }
    //is data none?
    if(!data || len == 0){
...
int rdx_ble_server_ota_send(u8 *data, u32 len)
{
    ...
    if (!g_rdx_ble_server_info.ble_con_handle ||
        !rdx_ble_session_is_rdx_active(
            g_rdx_ble_server_info.ble_con_handle)) {
        y_printf("[RDX_SESSION] OTA send skipped: capability not active\n");
        return -1;
    }
    if(!data || len == 0){
