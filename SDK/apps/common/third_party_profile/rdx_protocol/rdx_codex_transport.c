...
#pragma bss_seg(".rdx_codex_transport.data.bss")
#pragma data_seg(".rdx_codex_transport.data")
#pragma const_seg(".rdx_codex_transport.text.const")
#pragma code_seg(".rdx_codex_transport.text")
#endif
...
{
    u32 generation = (u32)priv;
    rdx_ble_async_token_t token;
    if (generation != s_codex_generation || !s_codex_rx.queued) {
        return;
    }
    token = s_codex_rx.token;
    if (!rdx_codex_token_resolve(&token)) {
        rdx_codex_rx_clear();
        return;
    }
    rdx_codex_micro_process_json(s_codex_rx.data, s_codex_rx.len);
    rdx_codex_rx_clear();
}
...
void rdx_codex_transport_deinit(void) {}
void rdx_codex_transport_runtime_reset(void) {}
u16 rdx_codex_transport_att_read(hci_con_handle_t c, u16 h, u16 o,
                                 u8 *b, u16 s)
{ (void)c; (void)h; (void)o; (void)b; (void)s; return 0; }
int rdx_codex_transport_output_write(hci_con_handle_t c, u16 o,
                                     const u8 *b, u16 s)
{ (void)c; (void)o; (void)b; (void)s; return -1; }
int rdx_codex_transport_send_json(const char *j) { (void)j; return -1; }
void rdx_codex_transport_on_can_send_now(void) {}
