...
/* forward declaration */
static void Transform (unsigned long *buf, unsigned long *in);
...
MD5Init (MD5_CTX *mdContext)
{
	...
	mdContext->buf[0] = (unsigned long)0x67452301UL;
	mdContext->buf[1] = (unsigned long)0xEFCDAB89UL;
	mdContext->buf[2] = (unsigned long)0x98BADCFEUL;
	mdContext->buf[3] = (unsigned long)0x10325476UL;
}
...
void MD5Final (unsigned char hash[], MD5_CTX *mdContext)
{
	...
	ByteToHexStr(mdContext->digest, (char*)hash, 16);
}

/* Basic MD5 step. Transforms buf based on in.
 */
static void
Transform (unsigned long *buf, unsigned long *in)
{
...
void md5_get(unsigned char encrypt[], unsigned char decrypt[])
{
	MD5_CTX md5;

	MD5Init(&md5);                  //��ʼ������md5���ܵĽṹ
	MD5Update(&md5, encrypt, (unsigned int)strlen((char*)encrypt));   //�������ܵ��ַ����м���
