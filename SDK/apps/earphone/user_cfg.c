...
#define USE_CONFIG_AUTO_OFF_SETTING          1
#else
...
#if (THIRD_PARTY_PROTOCOLS_SEL & RDX_EN)
    ret = syscfg_read(VM_RDX_AUTO_OFF_TIME, &auto_off_time, sizeof(AUTO_OFF_TIME_CONFIG));
    if (ret > 0 && auto_off_time.auto_off_time != 0xFFFFFFFF) {
        app_var.auto_off_time = auto_off_time.auto_off_time * 60;
    }else{
        app_var.auto_off_time = RDX_DEFAULT_SHUT_DOWN_TIME * 60; //TCFG_AUTO_SHUT_DOWN_TIME; //dons++
    }
...
//----------------------------------------------------------------------------------------
//dons++ 20250330 返回分钟
u32 sys_get_auto_off_time(void)
{
    int ret = 0;
    AUTO_OFF_TIME_CONFIG auto_off_time;
    ret = syscfg_read(VM_RDX_AUTO_OFF_TIME, &auto_off_time, sizeof(AUTO_OFF_TIME_CONFIG));
    if (ret > 0 && auto_off_time.auto_off_time != 0xFFFFFFFF) {
        app_var.auto_off_time = auto_off_time.auto_off_time * 60; //auto_off_time 单位：分钟
    }else{
        auto_off_time.auto_off_time = RDX_DEFAULT_SHUT_DOWN_TIME;
        app_var.auto_off_time = RDX_DEFAULT_SHUT_DOWN_TIME * 60; //TCFG_AUTO_SHUT_DOWN_TIME; //dons++
    }
    log_info("%s ===> auto_off_time: %d s \r", __func__, app_var.auto_off_time);
    return auto_off_time.auto_off_time; //mins
}

void sys_set_auto_off_time(u32 auto_off_time)
{
    int ret = 0;
    //auto_off_time 单位：分钟
    log_info("sys_set_auto_off_time --> auto_off_time: %d min", auto_off_time);
    
    AUTO_OFF_TIME_CONFIG auto_off_time_cfg;
    auto_off_time_cfg.auto_off_time = auto_off_time;
    ret = syscfg_write(VM_RDX_AUTO_OFF_TIME, &auto_off_time_cfg, sizeof(AUTO_OFF_TIME_CONFIG));
    if (ret > 0) {
        log_info("sys_set_auto_off_time success --> auto_off_time: %d mins \r", auto_off_time_cfg.auto_off_time);
        app_var.auto_off_time = auto_off_time * 60;
    }else{
