...
//                          SD NAND 上电自动格式化                                  //
//*********************************************************************************//
// T2620 的 TCFG_T2620_STORAGE_PRESERVE_ON_BOOT 优先于以下通用策略，禁止启动
// 自动/强制格式化。挂载失败保留原数据；首次制盘或恢复须显式执行格式化操作。
// 功能说明：优先以 SD0 文件系统是否 mount 成功为判断依据，mount 成功则跳过格式化；
// mount 失败时读取 VM 标志位：标志不存在表示首次初始化，标志有效表示已有文件系统
// 损坏，两种情况都会将 SD NAND 格式化为 FAT32。
//
// 格式化完成后必须重新 mount 成功才写入 VM 标志，避免格式化失败却被记录为完成。
// 用户通过 USB MSC 重新格式化后，只要 mount 成功就会直接跳过本流程。
//
// 注意：当前 OTA 与烧录升级不会擦除 VM，因此 VM 标志位在升级后仍然保留；若后续
// 升级策略变更会擦除 VM，下一次 mount 失败将按首次初始化路径处理。
