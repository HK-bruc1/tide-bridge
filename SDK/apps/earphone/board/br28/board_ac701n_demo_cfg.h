...
//*********************************************************************************//
#define NTC_DET_EN          0
#define NTC_POWER_IO        IO_PORTA_05 //IO_PORTC_03
#define NTC_DETECT_IO       IO_PORTA_06 //IO_PORTC_04
#define NTC_DET_AD_CH       (0x6)   //根据adc_api.h修改通道号

#define NTC_DET_UPPER        802//235  //正常范围AD值上限，0度时
#define NTC_DET_LOWER        304//34  //正常范围AD值下限，45度时
...
#define TCFG_HW_SPI2_PORT_DO                IO_PORTA_06//NO_CONFIG_PORT
#define TCFG_HW_SPI2_PORT_DI                NO_CONFIG_PORT//IO_PORTA_04//NO_CONFIG_PORT
...
//                          SD NAND 上电自动格式化                                  //
//*********************************************************************************//
// 功能说明：优先以 SD0 文件系统是否 mount 成功为判断依据，mount 成功则跳过格式化；
// mount 失败时再参考 VM 标志位，若 VM 也无效则判定为首次上电并自动格式化 SD NAND
// （保序原 FAT 子类型，如 FAT12→FAT12, FAT16→FAT16, FAT32→FAT32），格式化成功后
// 写入 VM 标志位并重新 mount。
//
// 适用场景：SD NAND 出厂已预格式化为 FAT 文件系统（通常为 FAT12 或 FAT16），
// 目前使用的是FAT文件系统：dev_reg.c:49
// mount 成功则直接使用，无需格式化；仅在文件系统损坏导致 mount 失败时触发自愈
// 格式化。用户通过 USB 大容量存储模式在 PC 上格式化后也不会被重复格式化。
//
// 注意 1：f_format("storage/sd0/C/", ...) 路径会保留原 FAT 子类型；若需强制格式
// 化为 FAT32，应改为 f_format("sd0", ...) 路径。
//
// 注意 2：当前 OTA 与烧录升级不会擦除 VM，因此 VM 标志位在升级后仍然保留；若后续
// 升级策略变更导致 VM 被擦除，则 SD0 可能再次触发格式化，需同步调整策略。
#define TCFG_SD0_FORMAT_ON_BOOT             ENABLE_THIS_MOUDLE

// SD0 强制格式化测试开关：仅用于测试 mount 失败后的自动格式化路径。
// 打开后无论 mount 是否成功，都会强制进入格式化分支（不用于量产！）。
#define TCFG_SD0_FORCE_FORMAT_ON_BOOT       DISABLE_THIS_MOUDLE

//*********************************************************************************//
//                          SD0 diagnosis / bring-up only                           //
//*********************************************************************************//
// SD NAND 诊断总开关：默认必须关闭，避免影响已调通芯片的正常 mount 时序。
// 打开后会在 mount 前后增加 raw open / ioctl / LBA0 读取日志，用于 bring-up 定位。
#define TCFG_SD0_DIAG_ENABLE                    DISABLE_THIS_MOUDLE
