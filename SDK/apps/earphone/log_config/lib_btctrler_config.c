...
const int CONFIG_DISTURB_SCAN_ENABLE = 0; //dons++
...
#if (TCFG_USER_BLE_ENABLE)
	#define DEFAULT_LE_FEATURES (LE_ENCRYPTION | LE_DATA_PACKET_LENGTH_EXTENSION | LL_FEAT_LE_EXT_ADV | LE_2M_PHY)
       ...
       #define LE_AUDIO_BIS_RX_LE_ROLE     0
		...
		const uint64_t config_btctler_le_features = LE_AUDIO_CIS_LE_FEATURES|DEFAULT_LE_FEATURES|RCSP_MODE_LE_FEATURES|LE_AUDIO_BIS_RX_LE_FEATURES; // | LE_DATA_PACKET_LENGTH_EXTENSION | LE_2M_PHY | LL_FEAT_LE_EXT_ADV;
	#else
...
const int config_bb_optimized_ctrl = VENDOR_BB_ISO_DIRECT_PUSH;//BIT(7);//|BIT(8);
	...
	const bool config_tws_le_role_sw = 0;    //是否支持ble跟随tws主从切换

	const int config_btctler_le_rx_nums = 40; //dons++ 20250925
