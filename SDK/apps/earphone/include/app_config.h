...
//--------------------------------------------------------------------------------------------------
#if (TCFG_THIRD_PARTY_PROTOCOLS_SEL == 0)
#undef  TCFG_THIRD_PARTY_PROTOCOLS_SEL
#define TCFG_THIRD_PARTY_PROTOCOLS_SEL                  RDX_EN
#endif
//--------------------------------------------------------------------------------------------------
...
#define TCFG_DEV_MANAGER_ENABLE					  			1  //dons++
#else
...
#elif TCFG_SD0_ENABLE || TCFG_SD1_ENABLE || TCFG_UDISK_ENABLE || TCFG_NOR_FAT || THIRD_PARTY_PROTOCOLS_SEL & RDX_EN
#define CONFIG_FATFS_ENABLE                                 1
#else
...
#define    TCFG_ENC_SPEEX_ENABLE     0
#elif (THIRD_PARTY_PROTOCOLS_SEL & (LL_SYNC_EN | CUSTOM_DEMO_EN | RDX_EN))
#define    OTA_TWS_SAME_TIME_ENABLE  (TCFG_USER_TWS_ENABLE)
#define    OTA_TWS_SAME_TIME_NEW     (TCFG_USER_TWS_ENABLE)     //使用新的tws ota流程
#define    TCFG_ENC_SPEEX_ENABLE     0
#else
...
#if TCFG_CFG_TOOL_ENABLE
#error "T2620 factory CDC conflicts with the online configuration tool"
#endif
#if !TCFG_T2620_PC_STORAGE_ENABLE || !TCFG_DIP_SWITCH_POWER_ENABLE || \
    !TCFG_USB_CDC_BACKGROUND_RUN || !TCFG_USB_SLAVE_CDC_ENABLE || \
    !TCFG_USB_SLAVE_ENABLE || !TCFG_USB_SLAVE_MSD_ENABLE || \
    !TCFG_PC_ENABLE || !TCFG_APP_PC_EN || !TCFG_SD0_ENABLE
#error "T2620 factory CDC requires DIP, SD0, PC/MSC and background USB slave CDC"
#endif
#if TCFG_USB_SLAVE_HID_ENABLE || TCFG_USB_SLAVE_AUDIO_SPK_ENABLE || \
    TCFG_USB_SLAVE_AUDIO_MIC_ENABLE || TCFG_USB_CUSTOM_HID_ENABLE || \
    TCFG_USB_SLAVE_MTP_ENABLE || TCFG_USB_SLAVE_MIDI_ENABLE || \
    TCFG_USB_SLAVE_PRINTER_ENABLE || TCFG_USB_DM_MULTIPLEX_WITH_SD_DAT0 || USB_PC_NO_APP_MODE
#error "T2620 factory CDC supports only separate CDC and SD0 MSC modes"
#endif
#endif
...
#define TCFG_LOWPOWER_RAM_SIZE				0//2	                // 低功耗掉电ram大小，单位：128K，可设置值：0、2、3
#else
...
#if (TCFG_THIRD_PARTY_PROTOCOLS_ENABLE && (TCFG_THIRD_PARTY_PROTOCOLS_SEL & (RCSP_MODE_EN | GFPS_EN | MMA_EN | FMNA_EN | REALME_EN | SWIFT_PAIR_EN | DMA_EN | CUSTOM_DEMO_EN | XIMALAYA_EN | AURACAST_APP_EN | RDX_EN))) || ((TCFG_LE_AUDIO_APP_CONFIG & (LE_AUDIO_UNICAST_SINK_EN | LE_AUDIO_JL_UNICAST_SINK_EN)))
