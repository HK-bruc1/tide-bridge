...
//  {"periph_demo",       3,     0,   512,   0 },
    ...
    // {"rdx_protocol_recv_task", 2,    0, 512,   1024},
    // {"rdx_app_audio_play_task",    3,     0,   256,   256 },
    {0, 0},
};
...
#define POWER_ON_KEY_DELAY_CNT                  (80)
...
void check_power_on_key(void)
{
    ...
    while (1) {
        ...
        if (get_power_on_status()) {
            putchar('+');
            delay_10ms_cnt++;
            if (delay_10ms_cnt > POWER_ON_KEY_DELAY_CNT) {
                app_var.poweron_reason = SYS_POWERON_BY_KEY;
                log_info("============= get_power_on_status\n");
                return;
            }
        } else {
